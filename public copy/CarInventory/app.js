...
/* Get autocomplete list for "year" values */
collectionRef.orderBy('year').get().then(function (querySnapshot) {
    /* get the data of all the documents into an array */
    var data = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().year;
    });
    /* get an array of distinct values */
...
/* Get autocomplete list for "year" values */
collectionRef.orderBy('price').get().then(function (querySnapshot) {
    /* get the data of all the documents into an array */
    var data = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().price;
    });
    /* get an array of distinct values */
    let price = [...new Set(data)];
    autocomplete(document.getElementById("price"), price);
}).catch(function (error) {
    console.log("Error getting documents: ", error);
});

/* Get autocomplete list for "year" values */
collectionRef.orderBy('mileage').get().then(function (querySnapshot) {
    /* get the data of all the documents into an array */
    var data = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().mileage;
    });
    /* get an array of distinct values */
    let mileage = [...new Set(data)];
    autocomplete(document.getElementById("mileage"), mileage);
}).catch(function (error) {
    console.log("Error getting documents: ", error);
});
...
/* Get autocomplete list for all autocomplete values for top searchbar*/
collectionRef.get().then(function (querySnapshot) {
    /* get the data of all the documents into arrays */
    var data = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().make;
    });
    var data2 = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().model;
    });
    var data3 = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().year;
    });
    var data4 = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().body;
    });
    var data5 = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().price;
    });
    var data6 = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().mileage;
    });
    var data7 = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().interior_color;
    });
    var data8 = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().exterior_color;
    });
    /** concatenating interior_color and exterior_color arrays 
     * so I can get a distinct array of their combined values later */
    var colors = data7.concat(data8);
    var data9 = querySnapshot.docs.map(function (documentSnapshot) {
        return documentSnapshot.data().used;
    });
    /* get an array of distinct values */
    let make = [...new Set(data)];
    let model = [...new Set(data2)];
    let year = [...new Set(data3)];
    let body = [...new Set(data4)];
    let price = [...new Set(data5)];
    let mileage = [...new Set(data6)];
    let color = [...new Set(colors)];
    let used = [...new Set(data9)];
    /* concatenating all arrays of distinct values into one search array */
    let search = make.concat(model, year, body, price, mileage, color, used);
...
function load() {
    ...
    if (form.make.value != '' ||
        ...
        console.log(searchArray.flat().indexOf('make'));
